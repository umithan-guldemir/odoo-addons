...
class AccountPayment(models.Model):
    ...
    destination_journal_ids = fields.Many2many(
        "account.journal", compute="_compute_destination_journals"
    )
    ...
    @api.depends(
        # "invoice_ids", # TODO: Find the new field name
        "payment_type",
        "partner_type",
        "partner_id",
    )
    def _compute_destination_account_id(self):
        ...
        for rec in self.filtered(
            ...
            partner = rec.partner_id.with_context(force_company=rec.company_id.id)
            if rec.partner_type == "customer":
                rec.destination_account_id = partner.property_account_receivable_id.id
            else:
                rec.destination_account_id = partner.property_account_payable_id.id

            if not (rec.partner_type and rec.destination_account_id):
                aml = rec.env["account.move.line"].search(
                    [
                        ("payment_id", "=", rec.id),
                        (
                            "account_id",
                ...
                if aml:
                    ...
                    rec.destination_account_id = (
                        ...
                        or rec.journal_id.default_credit_account_id
                    )
        return res
