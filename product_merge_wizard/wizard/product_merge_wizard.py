...
class ProductMergeWizard(models.TransientModel):    
        ...
        for table, column in self.get_fk_on("product_template"):
            ...
            if len(columns) <= 1:
                ...
                with mute_logger("odoo.sql_db"), self.env.cr.savepoint():
                    query = (
                        'UPDATE "%(table)s" SET %(column)s = %%s WHERE %(column)s = %%s'
                        % query_dic
                    )
                    self.env.cr.execute(
                        query,
    ...
    def _update_reference_fields(self, product_tmpl_id, new_product_tmpl_id):
        ...
        for record in record_ids:
            ...
            domain = [(record.name, "=", "product.template,%d" % product_tmpl_id.id)]
...
class ProductMergeAttributeLine(models.TransientModel):
    ...
    value_ids = fields.Many2many(
        "product.template.attribute.value",
        relation="product_merge_attr_val_rel2",
        string="Values",
...
class ProductMergeProductLine(models.TransientModel):
    ...
    value_ids = fields.Many2many(
        "product.template.attribute.value",
        relation="product_merge_product_att_val_rel2",
    )
            ...
            "domain": {
                "value_ids": [
                    (
                        "id",
                        "in",
                        self.wizard_id.attribute_line_ids.mapped("value_ids.id"),
                    ),
