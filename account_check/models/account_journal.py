...
class AccountJournal(models.Model):
    ...
    def get_journal_dashboard_datas(self):
        ...
        handed_checks = self.env["account.check"].search(domain_handed_issue_checks)
        holding_checks = self.env["account.check"].search(domain_holding_third_checks)

        num_checks_to_numerate = False
        if self.env["ir.actions.report"].search([("report_name", "=", "check_report")]):
        ...
        return dict(
            ...
            handed_amount=formatLang(
                self.env,
                sum(handed_checks.mapped("amount_company_currency")),
                currency_obj=self.company_id.currency_id,
            ),
            holding_amount=formatLang(
                self.env,
                sum(holding_checks.mapped("amount_company_currency")),
                currency_obj=self.company_id.currency_id,
            ),
        )
