...
class AccountPayment(models.Model):
            ...
            if rec.payment_method_code in ["issue_check"]:
                ...
                if sequence:
                    if rec.check_number != sequence.number_next_actual:
                        ...
                        sequence.sudo().write({"number_next_actual": rec.check_number})
    ...
    def onchange_date(self):
        if (
            self.check_issue_date
            and self.check_payment_date
            and self.check_issue_date > self.check_payment_date
        ):
    ...
    def do_checks_operations(self, vals=None, cancel=False):
        ...
        if (
            ...
            operation = "holding"
            if cancel:
                _logger.info("Cancelled Received Check")
            ...
            if len(inbound_method) == 1 and (
                ...
                if cancel:
                    _logger.info("Cancelled Transfered Check")
                ...
                if cancel:
                    _logger.info("Cancelled Sold Check")
                    rec.check_ids._del_operation(self)
                    return None
            ...
            # bank
                ...
                if cancel:
                    _logger.info("Cancelled Deposited Check")
                    rec.check_ids._del_operation(self)
                    return None
            ...
            if cancel:
                _logger.info("Cancelled Delivered Check")
                rec.check_ids._del_operation(self)
                return None
            ...
            if cancel:
                _logger.info("Cancelled Hand/debit Check")
                rec.check_ids._del_operation(self)
                rec.check_ids.unlink()
                return None
            ...
            if cancel:
                _logger.info("Cancelled Withdrawn Check")
                rec.check_ids._del_operation(self)
                rec.check_ids.unlink()
                return None

            _logger.info("Withdrawn Check")
    ...
    def do_print_checks(self):
        ...
        report_name = (
            len(checkbook) == 1
            and checkbook.report_template.report_name
            or "check_report"
        )
        check_report = (
    ...
    def _split_aml_line_per_check(self, move):
        ...
        if len(aml) > 1:
            raise UserError(_("Seems like this move has been already splited"))
        elif len(aml) == 0:
            raise UserError(_("There is not move lines to split"))

        amount_field = "credit" if aml.credit else "debit"
        new_name = _("Deposit check %s") if aml.credit else aml.name + _(" check %s")
